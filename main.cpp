...
Colour imageFunc(std::vector<Circle> circles, const double x, const double y, const int x_res, const int y_res) {
    ...
    for (int i = 0; i < circles.size(); i++) {
        const double centre_x = circles[i].centre.x;
        const double centre_y = circles[i].centre.y;

        const double dx = x - centre_x; 
        const double dy = y - centre_y;

        const double length_sq = dx * dx + dy * dy;
        const double radius_sq = circles[i].radius * circles[i].radius;
    ...
    for (int y = 0; y < y_res; y++) {
        for (int x = 0; x < x_res; x++) {
            ...
            for (int z = 0; z < num_samples; z++) {
                ...
                Colour c = imageFunc(circles, x + u, y + v, x_res, y_res);
                sum += c;
            }
            sum /= num_samples;
